...
export function MapSwitcher() {
  ...
  const handleSelect = useCallback(
    ...
    [mapId, markProbe, notifyUnavailable, setMap],
  );
  ...
  return (
    <div className="map-switcher" role="tablist" aria-label={t('toolbar.mapSelect')}>
      {MAPS.map((definition) => {
        ...
        const state = definition.id === mapId ? 'available' : probes.get(definition.id) ?? 'unknown';
        return (
          <button
            key={definition.id}
            type="button"
            ...
            title={
              state === 'preparing' ? t('switcher.preparing') : t(definition.nameKey)
            }
            className={
              `map-switcher-chip ${active ? 'active' : ''} ${state === 'preparing' ? 'preparing' : ''}`
            }
      ...
      {unavailableDefinition !== undefined ? (
        <span className="map-switcher-notice" role="status">
          ...
          <button type="button" aria-label={t('common.close')} onClick={() => notifyUnavailable(null)}>
            ×
          </button>
        </span>
      ) : null}
